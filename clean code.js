...
function removeEverything(){
    $('#resumeInfo').remove();
    var resInfo ='<div id="resumeInfo" class="col-10 col-sm-10 col-md-8 col-lg-8 mx-auto bg-primary mt-2 mt-sm-2 mt-md-0 b-rad-2 rela"></div>';
    $('#resume').append(resInfo);
}
...
function seperateValues(idOfForm, globalVar) {


    var jqueryID = '#' + idOfForm
    $(jqueryID + " input").each(function (child) {
        printValues(this.id, globalVar);
    })
...
function printValues(idOfInputs, globalVar) {
    ...
    if (slicedID == "newHead") {
        ...
        printInput(idOfInputs, globalVar);
    } else {

    }
    document.getElementById(idOfInputs).innerHTML;

}


function printHeadline(param, globalVar) {
...
function printInput(param,globalVar) {
    let inpValue = document.getElementById(param).value;
    let divinp = ' <div class="pb-2">' + inpValue + '</div>'
    let inpFinal = "#populateInput" + globalVar;

    $(inpFinal).append(divinp);

}


function printTitle(param,globalVar) {
    let titleValue = document.getElementById(param).value;
    let divTitle = ' <div class="pb-2">' + titleValue + '</div>'
    let titleFinal = "#titleResume" + globalVar;
